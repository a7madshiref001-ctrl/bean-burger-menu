/* ============================================================
   menu.js — bean&burger customer menu
   ...
   - SECURITY: every piece of venue-supplied data is escaped before it
...
(function () {
  ...
  var T = {
    ar: {
      ...
      all: 'الكل', lowcal: 'أقل من ٤٠٠ سعرة', spicy: 'حار', veg: 'نباتي',
      sar: 'ر.س', kcal: 'سعرة', empty: 'لا توجد أصناف مطابقة',
      sfdaHead: 'متوافق مع اشتراطات الهيئة العامة للغذاء والدواء',
      sfdaCal: 'السعرات', sfdaAl: 'مسببات الحساسية', sfdaNa: 'الصوديوم', sfdaSp: 'درجة الحرارة',
      sfdaNote: 'القيم الغذائية تقديرية لأغراض العرض، وتُعتمد نهائيًا من المطعم.',
      e_rating: 'التقييم', e_contact: 'تواصل', e_allergens: 'الحساسية', e_info: 'معلومات', e_location: 'الموقع',
      m_allergens: 'مسببات الحساسية', m_allergens_sub: 'المرجع القياسي لمسببات الحساسية الأربعة عشر. الرموز على كل صنف استرشادية وتُعتمد من المطعم.',
      m_info: 'معلومات المنيو', m_location: 'الموقع', m_contact: 'تواصل', m_rating: 'التقييم',
      info_body:
        '<p><span class="pill">{seal} متوافق مع اشتراطات هيئة الغذاء والدواء</span></p>' +
        '<p>يعرض هذا المنيو على كل صنف <strong>السعرات الحرارية</strong> و<strong>مسببات الحساسية</strong>، وفق اشتراطات الهيئة العامة للغذاء والدواء.</p>' +
        '<p class="note">القيم الغذائية المعروضة تقديرية لأغراض العرض وتُعتمد نهائيًا من المطعم. رموز مسببات الحساسية استرشادية وتُراجع من قبل المطعم.</p>',
      ...
      wa: 'واتساب',
      disc: 'القيم الغذائية تقديرية وتُعتمد من المطعم. الرموز التحذيرية لمسببات الحساسية استرشادية.'
    },
    en: {
      dir: 'ltr', langBtn: 'عربي',
      search: 'Search the menu…',
      all: 'All', lowcal: 'Under 400 kcal', spicy: 'Spicy', veg: 'Veggie',
      sar: 'SAR', kcal: 'kcal', empty: 'No matching items',
      sfdaHead: 'Compliant with SFDA menu-labeling requirements',
      sfdaCal: 'Calories', sfdaAl: 'Allergens', sfdaNa: 'Sodium', sfdaSp: 'Spice level',
      sfdaNote: 'Nutrition values are estimates for display and are confirmed by the restaurant.',
      e_rating: 'Rating', e_contact: 'Contact', e_allergens: 'Allergens', e_info: 'Info', e_location: 'Location',
      m_allergens: 'Allergens', m_allergens_sub: 'The standard 14-allergen reference. Icons on each item are indicative and confirmed by the restaurant.',
      m_info: 'Menu info', m_location: 'Location', m_contact: 'Contact', m_rating: 'Rating',
      info_body:
        '<p><span class="pill">{seal} SFDA-compliant labeling</span></p>' +
        '<p>Every item shows <strong>calories</strong> and <strong>allergens</strong>, per the Saudi Food &amp; Drug Authority requirements.</p>' +
        '<p class="note">Displayed nutrition values are estimates for display and are confirmed by the restaurant. Allergen icons are indicative and reviewed by the restaurant.</p>',
      ...
      wa: 'WhatsApp',
      disc: 'Nutrition values are estimates, confirmed by the restaurant. Allergen warning icons are indicative.'
    }
  };
  ...
  function load() {
    var override = null;
    try { override = localStorage.getItem('beanburger.menu'); } catch (e) {}
    if (override) {
  ...
  function badgesHTML(item) {
    ...
    if (item.spicy) {
      b += '<span class="badge salt">' + I.get('flame') + ' ' + (lang === 'ar' ? 'حار' : 'Spicy') + '</span>';
    }
    if (item.high_sodium) {
      b += '<span class="badge salt">' + I.get('salt') + ' ' + (lang === 'ar' ? 'صوديوم عالٍ' : 'High sodium') + '</span>';
    }
    return b;
  }
  ...
  function passes(item) {
    ...
    if (filter === 'lowcal' && !(item.kcal != null && item.kcal < 400)) return false;
  ...
  function buildStatic() {
    ...
    // hero tagline (logo itself is fixed in the HTML for both languages)
    $('heroTag').textContent = nm(b, 'tagline') || '';
    ...
    $('sfdaBadges').innerHTML =
      ...
      sfdaBadge('salt', ti.sfdaNa) + sfdaBadge('burger', ti.sfdaSp);
    $('sfdaNote').textContent = ti.sfdaNote;

    // controls
    $('miniBrand').innerHTML = I.get('burger');
    $('search').placeholder = ti.search;
    $('filterChips').innerHTML =
      filterChip('all', ti.all) + filterChip('spicy', ti.spicy) + filterChip('veg', ti.veg) + filterChip('lowcal', ti.lowcal);
  ...
  function openModal(kind) {
    ...
    if (kind === 'allergens') {
      ...
      body = '<div class="link-row">' +
        linkItem(b.instagram, 'instagram', ti.ig_label, '@' + (String(b.instagram || '').split('/').filter(Boolean).pop() || '')) +
        (b.tiktok ? linkItem(b.tiktok, 'tiktok', ti.tiktok_label, '') : '') +
        '</div>';
    } else if (kind === 'rating') {
